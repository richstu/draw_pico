...
//    unsigned int start = skip!=0 ?  0 : 1;
  ...
  //Note now photon_id80 is built into the nphoton variable in lassen_v0 picos and beyond
  ...
  const NamedFunc run2_baseline     = "nllphoton>0" && pass_dilepton_trigs_and_pt && "ll_m[0] > 50 && photon_pt[0]/llphoton_m[0] > 15.0/110 && ll_m[0] + llphoton_m[0] > 185 && llphoton_m[0] > 100 && llphoton_m[0] < 180 && pass";
  ...
  const NamedFunc wgt_fix("wgt_fix",[](const Baby &b) -> NamedFunc::ScalarType{ 
    if(b.SampleTypeString().Contains("-")) {
      return 1;
    }
    double w_year = w_years.GetScalar(b);
  ...
  void constructCutflowTable(std::vector<TableRow> & tablerows, NamedFunc weight, int electron_or_muon, bool isMC, NamedFunc current_cut) {
    ...
    if (electron_or_muon == 0) { // el
      ...
      tablerows = std::vector<TableRow>{
        TableRow("No selection",                                  current_cut,                                                                          0, 0, weight),
        TableRow("$N_e \\geq 2 || N_{\\mu} \\geq 2 $",            add_cut(current_cut, "nll>=1"),                                                       0, 0, weight),
        TableRow("e, ee trigger $||$ $\\mu, \\mu\\mu$ trigger",   current_cut && pass_trigs,                                                            0, 0, weight),
