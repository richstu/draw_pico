...
int main() {
  ...
  for (const auto& pair : SampleInfo) {
    ...
    if ((ProcInfo.proclegend.find("-ZH") != ProcInfo.proclegend.npos) || 
  ...
  pm.Push<Hist1D>(Axis(30,0,200,"met", "p_{T}^{miss} [GeV]", {}), bbgg_hh_baseline, procs_vec, ops).Weight(w_lumi_hb).Tag("ShortName:bbgg_hh_nm_met");
  pm.Push<Hist1D>(Axis(8,0,8,"nvlep", "Number of veto leptons", {1}), bbgg_hh_nm_nvl, procs_vec, ops).Weight(w_lumi_hb).Tag("ShortName:bbgg_hh_nm_nvlep");
  pm.Push<Hist1D>(Axis(8,0,8,"njet", "Number of jets", {2,4}), bbgg_hh_nm_njet, procs_vec, ops).Weight(w_lumi_hb).Tag("ShortName:bbgg_hh_nm_njet");
  pm.Push<Hist1D>(Axis(30,60,160,"bb_m", "M_{bb} [GeV]", {100,140}), bbgg_hh_nm_mbb, procs_vec, ops).Weight(w_lumi_hb).Tag("ShortName:bbgg_hh_nm_mbb");
  pm.Push<Hist1D>(Axis(30,0,4,"photonphoton_dr", "\\DeltaR_{\\gamma\\gamma}", {2.5}), bbgg_hh_nm_ggdr, procs_vec, ops).Weight(w_lumi_hb).Tag("ShortName:bbgg_hh_nm_ggdr");
  pm.Push<Hist1D>(Axis(30,0,300,"photonphoton_pt", "p_{T}^{\\gamma\\gamma} [GeV]", {75}), bbgg_hh_nm_ggpt, procs_vec, ops).Weight(w_lumi_hb).Tag("ShortName:bbgg_hh_nm_ggpt");
  pm.Push<Hist1D>(Axis(20,100,200,"photonphoton_m", "M_{\\gamma\\gamma} [GeV]", {}), bbgg_hh_nm_ggm, procs_vec, ops).Weight(w_lumi_hb).Tag("ShortName:bbgg_hh_nm_ggm");
  pm.Push<Hist1D>(Axis(20,-1,1,"photon_idmva[0]", "Photon mva id of lead photon", {}), bbgg_hh_nm_photonidlead, procs_vec, ops).Weight(w_lumi_hb).Tag("ShortName:bbgg_hh_nm_photonidlead");
  pm.Push<Hist1D>(Axis(20,-1,1,"photon_idmva[1]", "Photon mva id of sublead photon", {}), bbgg_hh_nm_photonidsublead, procs_vec, ops).Weight(w_lumi_hb).Tag("ShortName:bbgg_hh_nm_photonidsublead");
