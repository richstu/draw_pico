...
int main() {
  ...
  tablevec.push_back(TableRow("No cuts", "1", 0, 0, w_lumi_hb)); //first row without any cuts
  tablevec.push_back(TableRow("sig_decay_bbgg", sig_decay_bbgg, 0, 0, w_lumi_hb)); 
  // Iterate over the sample info map
  for (const auto& pair : cuts_bbgg_hh) {
    ...
    tablevec.push_back(TableRow(cutlabel,cuts.str() && sig_decay_bbgg, 0, 0, w_lumi_hb));
  }

  //push special cuts not in the cut vector
  tablevec.push_back(TableRow("trigger", cuts.str() && sig_decay_bbgg && diphoton_trig, 0, 0, w_lumi_hb));  
  tablevec.push_back(TableRow("deepflavmedium_lead", cuts.str() && sig_decay_bbgg && diphoton_trig && DeepFlav_leadjet, 0, 0, w_lumi_hb));
  tablevec.push_back(TableRow("deepflavmedium_sublead", cuts.str() && sig_decay_bbgg && diphoton_trig && DeepFlav_leadjet && DeepFlav_subleadjet, 0, 0, w_lumi_hb));

  pm.Push<Table>("Cutflow_bbgg_hh", tablevec, procs_vec, false, true, false, false, false, false); //do_zbi, print_table, print_pie, print_titlepie, do_eff, do_unc
  pm.Push<Table>("Cutflow_bbgg_hh_eff", tablevec, procs_vec, false, true, false, false, true, false); //do_zbi, print_table, print_pie, print_titlepie, do_eff, do_unc
