...
const Content& Category::child(const std::vector<Variable::Type>& values) const {
  auto pval_s = std::get_if<std::string>(&values[variableIdx_]);
  auto pval_i = std::get_if<int>(&values[variableIdx_]);
  if ( pval_s ) {
    try {
      return std::get<StrMap>(map_).at(*pval_s);
    } catch (std::out_of_range& ex) {
      if ( default_ ) {
        return *default_;
      }
      else {
        throw std::out_of_range("Index not available in Category for input argument " + std::to_string(variableIdx_) + " val: " + *pval_s);
      }
    }
  }
  else if ( pval_i ) {
    try {
      return std::get<IntMap>(map_).at(*pval_i);
    } catch (std::out_of_range& ex) {
      if ( default_ ) {
        return *default_;
      }
      else {
        throw std::out_of_range("Index not available in Category for input argument " + std::to_string(variableIdx_) + " val: " + std::to_string(*pval_i));
      }
    }
  }
