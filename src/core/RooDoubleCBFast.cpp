...
//hack remove vdtmath dependency and manually insert relevant functions
...
inline double fast_pow(double base, double exponent) {
  ...
  else if (base>0.) return exp(exponent*log(base));
  else return std::nan("");
}
// /hack
...
using namespace RooFit;
 ...
 { 
   double t = (x-mean)*inv(width);
   double val = -99.;
   if(t>-alpha1 && t<alpha2){
     val = exp(-0.5*t*t);
   }else if(t<=-alpha1){
     double alpha1invn1 = alpha1*inv(n1);
     val = exp(-0.5*alpha1*alpha1)*fast_pow(1. - alpha1invn1*(alpha1+t), -n1);
     
//      double n1invalpha1 = n1*inv(fabs(alpha1));
//      double A1 = fast_pow(n1invalpha1,n1)*exp(-alpha1*alpha1/2.);
//      double B1 = n1invalpha1-fabs(alpha1);
//      val = A1*fast_pow(B1-t,-n1);
   }else if(t>=alpha2){
     double alpha2invn2 = alpha2*inv(n2);
     val = exp(-0.5*alpha2*alpha2)*fast_pow(1. - alpha2invn2*(alpha2-t), -n2);     
     
//      double n2invalpha2 = n2*inv(fabs(alpha2));
//      double A2 = fast_pow(n2invalpha2,n2)*exp(-alpha2*alpha2/2.);
...
//      val = A2*fast_pow(B2+t,-n2);
 ...
 Double_t RooDoubleCBFast::analyticalIntegral(Int_t code, const char* rangeName) const 
 {
   ...
   if (isfullrange  && (n1-1.0)>1.e-5) {
    left = width*exp(-0.5*alpha1*alpha1)*n1*inv(alpha1*(n1-1.)); 
   }
   else {
	...
	//left = width*exp(-0.5*alpha1*alpha1)*invn1m1*(n1invalpha1 -  fast_pow( fast_pow(n1invalpha1,-n1*invn1m1)*(n1invalpha1 - alpha1 - tmin), 1.-n1)) ;
	//left = width*exp(-0.5*alpha1*alpha1)*invn1m1*(n1invalpha1 -  fast_pow( fast_pow(n1invalpha1,-n1*invn1m1)*(n1invalpha1 - alpha1 - tmin), 1.-n1)) ;
	//left = A1*inv(-n1+1.0)*width*(fast_pow(B1-(left_low-mean)*invwidth,-n1+1.)-fast_pow(B1-(left_high-mean)*invwidth,-n1+1.));
      }
      else {
	double A1 = fast_pow(n1invalpha1,n1)*exp(-0.5*alpha1*alpha1);
	double B1 = n1invalpha1-fabs(alpha1);	
	left = A1*width*(log(B1-(left_low-mean)*invwidth) - log(B1-(left_high-mean)*invwidth) );
      }
    }
   }
   ...
   if (isfullrange && (n2-1.0)>1.e-5) {
     right = width*exp(-0.5*alpha2*alpha2)*n2*inv(alpha2*(n2-1.));
   }
   else {    
	...
	//right = A2*inv(-n2+1.0)*width*(fast_pow(B2+(right_high-mean)*invwidth,-n2+1.)-fast_pow(B2+(right_low-mean)*invwidth,-n2+1.));
      }
      else {
	double A2 = fast_pow(n2invalpha2,n2)*exp(-0.5*alpha2*alpha2);
	double B2 = n2invalpha2-fabs(alpha2);
	right = A2*width*(log(B2+(right_high-mean)*invwidth) - log(B2+(right_low-mean)*invwidth) );
      }
    }
   }
