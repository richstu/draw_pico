  ...
  x("_index","the pdf index",this,_x) 
{
  ...
  RooAbsPdf *fPdf;
  while ( (fPdf = static_cast<RooAbsPdf*>(pdfIter->Next())) ){
	c.add(*fPdf);
...
double RooMultiPdf::getCorrection() const {
  ...
  double val = (static_cast<RooAbsReal*>(corr.at(x)))->getVal(); 
  return cFactor*val;  //PVAL correction
}
//_____________________________________________________________________________
RooAbsPdf* RooMultiPdf::getCurrentPdf() const {

  RooAbsPdf *cPdf = (static_cast<RooAbsPdf*>(c.at(x))); 
  return cPdf; 
}
RooAbsPdf* RooMultiPdf::getPdf(int index) const {
    
  RooAbsPdf *cPdf = (static_cast<RooAbsPdf*>(c.at(index)));
  return cPdf;
}
...
//_____________________________________________________________________________
[[noreturn]] Double_t RooMultiPdf::evaluate() const{
