...
if __name__ == '__main__':
  ...
  parser.add_argument('-p','--png_format', action="store_true", required=False, help='Generate PNG')

  args = parser.parse_args()

  # https://github.com/rrthomas/pdfjam/releases/download/v3.10/pdfjam-3.10.tar.gz
  pdfjam_command = './scripts/pdfjam' 
  ...
  # Trim pdf
  command = f'gs -o {args.output_pdf} -sDEVICE=pdfwrite -dCompatibilityLevel=1.5 -c "[/CropBox [{lower_x} {lower_y} {upper_x} {upper_y}] /PAGES pdfmark" -f {args.output_pdf}.1;imgcat {args.output_pdf}'
  print(command)
  os.system(command)
  ...
  if(args.png_format):
    print("Making PNG of PDF")
    output_name = args.output_pdf
    png_filename = args.output_pdf.replace('.pdf','')
    command = f'pdftoppm -png {args.output_pdf} {png_filename}'
    os.system(command)
    command = f'rm {output_name}'
    os.system(command)
    command = f'mv {png_filename}-1.png {png_filename}.png'
    os.system(command)
